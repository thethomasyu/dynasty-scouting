...
import { evalDateFor } from '../data/players'
import { ageAt, formatDob, formatHeight } from '../lib/bio'
import { positionLabel } from '../lib/routes'
...
function BioStrip({ bio, evalDate }: { bio: PlayerBio; evalDate: string }) {
  const listed = bio.measurementStatus === 'school-listed'
  const age = ageAt(bio.dateOfBirth, evalDate)
...
export default function PlayerHero({ player }: { player: Player }) {
  const cutout = cutoutOf(player.slug)
  return (
    <section
      className={`p-hero on-ink${cutout ? '' : ' p-hero--nostage'}`}
      style={{ '--school': player.accent } as React.CSSProperties}
      aria-label={`${player.name}, ${player.school}, ${positionLabel(player.position)}`}
    >
      ...
      <div className="wrap p-hero__inner">
        <div className="p-hero__copy">
          <p className="kicker kicker--dot p-hero__kicker">
            2027 {player.position} class · {player.school}
          ...
          <div className="p-hero__meta">
            ...
            {player.provisional && (
              <span className="p-hero__badge p-hero__badge--provisional">Provisional</span>
            )}
          </div>
          {player.bio && <BioStrip bio={player.bio} evalDate={evalDateFor(player.position)} />}
